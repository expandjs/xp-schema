...
module.exports.sanitizers = {
    ...
    list(value, bool) {
        ...
        if (bool && XP.isString(value)) { return value.split(/\s*,\s*/); }
     ...
     */
    type(value, type) {
        if (['boolean', 'date', 'number', 'string'].includes(type)) { value = XP.toPrimitive(value, type, true); }
        if (['boolean'].includes(type)) { return value || false; }
        return XP.toDefined(value);
    }
};
