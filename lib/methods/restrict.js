...
module.exports = function (schema, item, options) {
    ...
    Object.keys(item).forEach(name => {
        ...
        if (!field.list) { return XP.isObject(step) && module.exports(field.recursive ? schema : field, step); }

        // Restricting (sub values)
        Array.from(XP.isArray(step) ? step : []).forEach(value => XP.isObject(value) && module.exports(field.recursive ? schema : field, value));
    });
