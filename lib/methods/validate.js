...
module.exports.validators = {
    ...
    options(value, opts, path) {
        ...
        if (!opts.includes(value)) { return XP.error(400, `${path ? `"${path}"` : `item`} can't be "${value}".`); }
        return null;
    },
    ...
    pattern(value, pattern, path) {
        ...
        if (!XP.isVoid(pattern = XP.toRegExp(pattern)) && !pattern.test(value)) { return XP.error(400, `${path ? `"${path}"` : `item`} can't be "${value}".`); }
        return null;
    },
