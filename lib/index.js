...
module.exports = env.XPSchema = new XP.Class('XPSchema', {
     ...
     * @method filter
     * @param {Object} item
     * @param {boolean} [writing = false]
     * @param {Function} [callback]
     */
    filter: {
        callback: true,
        value(item, writing, callback) {
            XP.waterfall([
                next => this._assert({item: item}, next), // asserting
                next => next(null, filter(this, item, {writing: writing})) // filtering
            ], callback);
        }
    },
    ...
    map: {
        callback: true,
        value(item, reversed, callback) {
            XP.waterfall([
                next => this._assert({item: item}, next), // asserting
                next => next(null, this.mapped ? map(this, item, {reversed: reversed}) : item) // mapping
            ], callback);
        }
    },
    ...
    restrict: {
        ...
        value(item, reversed, callback) {
            XP.waterfall([
                next => this._assert({item: item}, next), // asserting
                next => next(null, restrict(this, item, {reversed: reversed})) // restricting
            ], callback);
        }
    },
