...
module.exports = new XP.Class('XPSchema', {
    ...
    filter: {
        ...
        value(item, options, callback) {
            XP.waterfall([
                ...
                next => next(null, filter(this, item, options)) // filtering
            ], callback);
        }
    },
    ...
    map: {
        ...
        value(item, options, callback) {
            XP.waterfall([
                ...
                next => next(null, this.mapped ? map(this, item, options) : item) // mapping
            ], callback);
        }
    },
    ...
    restrict: {
        ...
        value(item, options, callback) {
            XP.waterfall([
                next => this._assert({item, options}, next), // asserting
                next => next(null, restrict(this, item, options)) // restricting
            ], callback);
        }
    },
    ...
    _assertItem: {enumerable: false, value(val) { return !XP.isObject(val) && XP.ValidationError('item', 'Object'); }},
    _assertOptions: {enumerable: false, value(val) { return !XP.isVoid(val) && !XP.isObject(val) && XP.ValidationError('options', 'Object'); }}
});
